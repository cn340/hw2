...
<details>
  ...
  <p><blockquote>If you are developing on PaizaCould, the URL is something like <code>https://localhost-your-workspace-name.paiza-user.cloud:9292/new</code>.</blockquote></p>
</details>
<br />
...
* Verify that the Heroku-deployed Hangperson behaves the same as your development version before continuing. A few lines up from the bottom of the Heroku output in the terminal should have a URL ending in herokuapp.com. Find that, copy it to the clipboard, and paste it into a browser tab to see the current app. The PaizaCloud IDE browser tab won't render the app, so use a new browser tab outside of PaizaCloud.
